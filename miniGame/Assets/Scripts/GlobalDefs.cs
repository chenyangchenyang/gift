...
public class GlobalDefs : MonoBehaviour {
    ...
    private GameObject leavehint, letterhint;
	...
	void Start () {
        ...
        letterhint = GameObject.Find("letterhint");
        if (letterhint != null)
	...
	void Update () {
        ...
        if (GlobalTool.needCheckLetterHint)
        {
            if (GlobalTool.idleTime > 4)
            {
                GlobalTool.idleTime = -2;
                letterhint.SetActive(true);
                letterhint.GetComponent<Animation>().Play("hint-left");
                Invoke("HideLetterHint", 2);
            }
        }
    }
